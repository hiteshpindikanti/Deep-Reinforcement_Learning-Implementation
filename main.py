...
if __name__ == '__main__':
    ...
    learning_rate = 1e-4
    render = False
    load_prev_game_data = True

    if load_prev_game_data:
        with open('game_data.dat') as f:
            episode_number,\
            reward_sum,\
            running_reward,\
        ...
        reward_sum=0
        prev_processed_observations = prev_processed_observations.tolist()
        episode_observations = episode_observations.tolist()
        episode_hidden_layer_values = episode_hidden_layer_values.tolist()
        #episode_observations = episode_observations.tolist()
        episode_gradient_log_ps = episode_gradient_log_ps.tolist()
    ...
    while True:
        ...
        if done: # an episode finished
            ...
            if episode_number % batch_size == 0:
                ...
                with open('game_data.dat', 'w') as f:
                    pickle.dump([episode_number,\
                    reward_sum,\
                    running_reward,\
                    prev_processed_observations,\
                    weights,\
                    ...
                    episode_observations,\
                    episode_gradient_log_ps,\
                    episode_rewards], f, pickle.HIGHEST_PROTOCOL)
                print '---Batch completed, updated weights and saved variables to file--'

            episode_hidden_layer_values, episode_observations, episode_gradient_log_ps, episode_rewards = [], [], [], [] # reset values
            observation = env.reset() # reset env
            running_reward = reward_sum if running_reward is None else running_reward * 0.99 + reward_sum * 0.01
            print 'episode: %d, episode reward total: %f, running mean: %f' % (episode_number,reward_sum, running_reward)

            #saving results
            with open('game_results.txt', 'a') as f:
                pickle.dump([episode_number, reward_sum, running_reward], f)
            reward_sum = 0
            prev_processed_observations = None
